...
async function backfillRunSnapshotsIfMissing(runId) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const ir of missingItems) {
      ...
      await tx.checklistItemRun.update({
        where: { id: ir.id },
        data: {
...
checklistRoutes.get("/run/:runId", async (req, res) => {
...
checklistRoutes.post("/start", async (req, res) => {
  const body = z.object({
    companyId: z.string(),
  ...
  for (const { sectionName, it } of items) {
    const ir = await prisma.checklistItemRun.create({
      data: {
        runId: run.id,
  ...
  await audit(req, "CHECKLIST_START", "ChecklistRun", run.id, undefined, body.data);
  res.status(201).json({ runId: run.id, templateId: template.id, itemRunMap });
});

checklistRoutes.patch("/item/:itemRunId", async (req, res) => {
  const body = z.object({
  ...
  const itemRun = await prisma.checklistItemRun.findUnique({
  ...
  const updated = await prisma.checklistItemRun.update({ where: { id: itemRun.id }, data });

  // Anchor: the checklist starts when the first item is marked as CONCLUIDO.
  // (Users expect "any first check" to start, not only "required" items.)
  if (normalizeStatus(body.data.status) === "CONCLUIDO") {
    const run = await prisma.checklistRun.findUnique({ where: { id: itemRun.runId } });
    if (run && !run.anchorAt) {
      const anchorAt = updated.doneAt || new Date();
      await prisma.checklistRun.update({ where: { id: run.id }, data: { anchorAt } });

      const runItems = await prisma.checklistItemRun.findMany({
        where: { runId: run.id },
        select: {
          id: true,
          snapshotDueRuleType: true,
      ...
      for (const ri of runItems) {
        ...
        if (due) await prisma.checklistItemRun.update({ where: { id: ri.id }, data: { dueDate: due } });
      }
    }
  }

  await audit(req, "CHECKLIST_ITEM_UPDATE", "ChecklistItemRun", updated.id, itemRun, updated);
  res.json({ ok: true });
});
